//Crie uma lista encadeada de alunos com a idade do aluno, ordene a lista utilizando o algoritmo selection sort.
#include <stdio.h>
#include <stdlib.h>
#include <locale.h>
#include <string.h>
...
void Ordena(Aluno* lista) {
  ...
  for (i = lista; i != NULL && i->prox != NULL; i = i->prox) {
    min = i;
    for (j = i->prox; j != NULL; j = j->prox) {
      if (j->idade < min->idade) {
        min = j;
      }
    }
    if (min != i) {
      ...
      strcpy(i->nome, min->nome);
      strcpy(min->nome, NomeTemp);
    }
  }
}
