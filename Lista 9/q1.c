...
// – Crie uma função para remover elementos pelo index(remoção entre elementos).
...
// - Inserir aluno;
// - Remover Aluno (informa o index);
// - Buscar Aluno;
// - Calcular média;
// - Lista Alunos (Nome, Matrícula);
// - Sair;
...
void remover(Aluno** lista, int index) {
    if (*lista == NULL)
        return;
    Aluno* atual = *lista;
    Aluno* anterior = NULL;
